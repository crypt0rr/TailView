...
export function Operations() {
  ...
  return <div className="page operations-page">
    ...
    {tab === "storage" && (storage.isLoading ? <Loading /> : storage.error ? <ErrorState error={storage.error} /> : <div className="operations-grid operations-storage-grid">
      <Card className="table-card operations-storage-card">
        <CardHead title="Managed records" detail="Counts by retained data class." />
        <div className="managed-records-list">
          {Object.entries(storage.data?.counts ?? {}).map(([name, count]) => <div className="setting-row" key={name}><strong>{name.replaceAll("_", " ")}</strong><span>{count.toLocaleString()}</span></div>)}
        </div>
      </Card>
      <Card className="table-card operations-storage-card">
        <CardHead title="PostgreSQL relations" detail="Table and index usage; host filesystem capacity is deliberately not inferred." />
        {!storage.data?.relations.length ? <Empty title="Relation sizes unavailable" detail="Detailed relation sizes require PostgreSQL." /> : <div className="table-scroll" tabIndex={0}><table><thead><tr><th>Relation</th><th>Total</th><th>Table</th><th>Indexes</th></tr></thead><tbody>{storage.data.relations.map((row) => <tr key={row.name}><td>{row.name}</td><td>{formatBytes(row.total_bytes)}</td><td>{formatBytes(row.table_bytes)}</td><td>{formatBytes(row.index_bytes)}</td></tr>)}</tbody></table></div>}
      </Card>
    </div>)}
