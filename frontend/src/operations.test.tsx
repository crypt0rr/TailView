...
describe("operations center", () => {
  it("shows degraded runtime, queues, retention protection, and recovery evidence", async () => {
    vi.spyOn(apiModule, "request").mockImplementation(async (path) => {
      ...
      if (path === "/operations/backups") return { items: [{ id: "backup-1", filename: "tailview.dump", content_hash: "a".repeat(64), size: 1024, status: "success", postgres_version: "17.10", migration_revision: "0014_v1_completion", checks: { restore: true }, error_class: "", verified_at: summary.backup.latest_verified_at }] } as never;
